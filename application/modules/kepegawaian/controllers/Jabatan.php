...
class Jabatan extends MX_Controller
{
    ...
    {
        $this->output_json($this->M_karyawan->getDataJabatan($nip), false);
    }
    ...
    public function tambah($id, $nip)
    {
        ...
        $data['data'] = $this->M_karyawan->data_jabatan($nip);
        ...
        $data['modal'] = true;
        $data['content'] = 'karyawan/jabatan/v_modal_add';
    ...
    public function lihat($id)
    {
        ...
        $data['data'] = $this->M_karyawan->detail_jabatan($id);
        $data['combolembaga'] = $this->M_combo->combolembaga($idlembaga);
        $data['combojabatan'] = $this->M_combo->combojabatan();
        $data['combounitkerja'] = $this->M_combo->combounitkerja();

        $data['modal'] = true;
        $data['content'] = 'karyawan/jabatan/v_modal_view';
        $this->load->view('layout/main', $data);
            ...
            if (!empty($_FILES['urlpic']['name'])) {
                if ($this->upload->do_upload('urlpic')) {
                    ...
                    $data = array(
                        ...
                        'no_sk' => $nosk,
                        'tgl_sk' => $this->M_karyawan->rubah_tglformat($tglsk),
                        'tmt_jabatan' => $this->M_karyawan->rubah_tglformat($tmtjabatan),
                        'url_sk' => $url,
                    );

                    $hasil = $this->M_karyawan->simpan_jabatan($data);
    ...
    public function ubah($id)
    {
        ...
        $data['combounitkerja'] = $this->M_combo->combounitkerja();
        $data['modal'] = true;
        $data['content'] = 'karyawan/jabatan/v_modal_edit';
        $this->load->view('layout/main', $data);
    ...
    public function update()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $idjabkaryawan = $this->input->post('idjabkaryawan');
            $idjabatan = $this->input->post('idjabatan');
            $idunitkerja = $this->input->post('idunitkerja');
            $tmtjabatan = $this->input->post('tmtjabatan');
            $nosk = $this->input->post('nosk');
            ...
            if (!empty($_FILES['urlpic']['name'])) {
                if ($this->upload->do_upload('urlpic')) {
                    ...
                    $hasil = $this->M_karyawan->ubah_jabatan($idjabkaryawan,$data);
    ...
    public function hapus($id)
    {
        ...
        $hasil = $this->M_karyawan->hapus_jabatan($id);
        if ($hasil == true) {
            $this->output_json(['status' => true]);
        } else {
            $this->output_json(['status' => false]);
     ...
     public function _validate($method)
    {
        ...
        $this->form_validation->set_rules('nosk', 'No SK', 'required|trim');
